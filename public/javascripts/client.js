(function() {
  ...
  Client = (function() {
    ...
    Client.prototype.movePaddle = function(paddle, direction) {
      return this.socket.broadcast.to(this.channel).emit("movePaddle", {
        paddle: paddle,
        direction: direction
      });
    };
    ...
    Client.prototype.disconnect = function() {
      if (Logger.info) {
        Logger.info("Client disconnected from channel " + this.channel + ".");
      }
      if (this.nickname) {
        return this.socket.broadcast.to(this.channel).emit("channel message", {
          userName: "Server",
